  ...
  Ownable
{
   ...
   */
  function calculateAnchorPriceFromEthPrice(
    TokenConfig memory config
  ...
  function getUniswapTwap(
    ...
    (int56[] memory tickCumulatives, ) = IUniswapV3Pool(config.uniswapMarket)
      .observe(secondsAgos);
  ...
  function fetchEthPrice() internal view returns (uint256) {
    return
      fetchAnchorPrice(getTokenConfigBySymbolHash(ETH_HASH), ETH_BASE_UNIT);
  }
